...
import styles from "./PricingApproach.module.scss";
...
export const PricingApproach = () => {
  ...
  return (
    <motion.section
      className={styles.pricing_approach}
      initial="hidden"
      whileInView="visible"
      viewport={{ once: true, amount: 0.1 }}
      variants={staggerContainer}
    >
      <div className={styles.pricing_approach__container}>
        {/* Left: icon + label */}
        <motion.div
          className={styles.pricing_approach__left}
          variants={staggerItem}
        >
          <span className={styles.pricing_approach__icon}>{CHECK_ICON}</span>
          <span className={styles.pricing_approach__label}>
            {t("approachLabel", { fallback: "APPROACH" })}
          </span>
        </motion.div>

        {/* Right: text */}
        <motion.div
          className={styles.pricing_approach__right}
          variants={staggerItem}
        >
          <h2 className={styles.pricing_approach__title}>
            {t("approachTitle", {
              fallback:
                "We believe in transparent and fair pricing. The more comprehensive the solution you choose, the more value you get — complex projects come with built-in discounts.",
            })}
          </h2>
          <p className={styles.pricing_approach__desc}>
            {t("approachDesc", {
              fallback:
                "Every service has a clear cost, so you can see exactly what you're paying for. By combining multiple services, you benefit from savings while getting a complete, tailored solution for your website, hosting, maintenance, and support.",
            })}
          </p>
        </motion.div>
      </div>
