...
import styles from "./SamPricingCta.module.scss";
...
export const SamPricingCta = () => {
  const t = useTranslations("sam");

  return (
    <section className={styles.sam_pricing_cta}>
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img
        src="/images/hosting-solutions/pricing-cta-bg.png"
        alt=""
        className={styles.sam_pricing_cta__bg}
      />
      <motion.div
        className={styles.sam_pricing_cta__container}
        initial="hidden"
        animate="visible"
        variants={fadeInUpLyniq}
      >
        <h2 className={styles.sam_pricing_cta__title}>
          {t("pricingCtaTitle", { fallback: "Need a full pricing list?" })}
        </h2>
        <Link href="/connect" className={styles.sam_pricing_cta__link}>
          <span className={styles.sam_pricing_cta__link_icon}>
            {ARROW_ICON}
          </span>
