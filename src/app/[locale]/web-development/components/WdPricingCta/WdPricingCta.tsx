...
import styles from "./WdPricingCta.module.scss";
...
export const WdPricingCta = () => {
  const t = useTranslations("webDev");

  return (
    <section className={styles.wd_pricing_cta}>
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img
        src="/images/web-development/hero-bg.png"
        alt=""
        className={styles.wd_pricing_cta__bg}
      />
      <motion.div
        className={styles.wd_pricing_cta__container}
        initial="hidden"
        animate="visible"
        variants={fadeInUpLyniq}
      >
        <h2 className={styles.wd_pricing_cta__title}>
          {t("pricingCtaTitle", { fallback: "Need a full pricing list?" })}
        </h2>
        <Link href="/connect" className={styles.wd_pricing_cta__link}>
          <span className={styles.wd_pricing_cta__link_icon}>{ARROW_ICON}</span>
